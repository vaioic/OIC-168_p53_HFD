...
def cellpose = Cellpose2D.builder( pathModel )
        .pixelSize( 0.1725 )                  // Resolution for detection in um
        .channels( 0,1 )	               // Select detection channel(s)
        .tempDirectory( new File( "/varidata/research/projects/pospisilik/vari-core-generated-data/OIC/Ilaria/OIC-168_p53HFD_PaperRevisions/p53/cellpose-temp02" ) ) // Temporary directory to export images to. defaults to 'cellpose-temp' inside the QuPath Project
...
//        .simplify( 0 )                     // Simplification 1.6 by default, set to 0 to get the cellpose masks as precisely as possible
        .addParameter('gpu_device','0')
        .build()

// Run detection for the selected objects
def imageData = getCurrentImageData()
//def pathObjects = getSelectedObjects() // To process only selected annotations, useful while testing
def pathObjects = getAnnotationObjects().findAll {it.getPathClass() != getPathClass('LU_Training')} // To process all annotations. For working in batch mode
...
runObjectClassifier("LU_p53_Nuc_Filter");
def toDelete = getDetectionObjects().findAll {it.getPathClass() == getPathClass('Toss')}
removeObjects(toDelete, true);
resetDetectionClassifications();
selectDetections();
runPlugin('qupath.lib.algorithms.IntensityFeaturesPlugin', '{"pixelSizeMicrons":0.1725,"region":"ROI","tileSizeMicrons":25.0,"channel1":false,"channel2":true,"doMean":false,"doStdDev":false,"doMinMax":false,"doMedian":false,"doHaralick":true,"haralickMin":0.0,"haralickMax":7000.0,"haralickDistance":1,"haralickBins":32}')
runObjectClassifier("LU_p53");
