...
def cellpose = Cellpose2D.builder( pathModel )
        ...
        .tempDirectory( new File( '/varidata/research/projects/pospisilik/vari-core-generated-data/OIC/Ilaria/OIC-168_p53HFD_PaperRevisions/p53/cellpose-temp01' ) ) // Temporary directory to export images to. defaults to 'cellpose-temp' inside the QuPath Project
...
def pathObjects = getAnnotationObjects().findAll {it.getPathClass() != getPathClass('Training')} // To process all annotations. For working in batch mode
...
runObjectClassifier("p53_Nuc_Filter");
selectDetections();
runPlugin('qupath.lib.algorithms.IntensityFeaturesPlugin', '{"pixelSizeMicrons":0.1725,"region":"ROI","tileSizeMicrons":25.0,"channel1":false,"channel2":true,"doMean":false,"doStdDev":false,"doMinMax":false,"doMedian":false,"doHaralick":true,"haralickMin":0.0,"haralickMax":12000.0,"haralickDistance":1,"haralickBins":32}')
def toDelete = getDetectionObjects().findAll {it.getPathClass() == getPathClass('Toss')}
removeObjects(toDelete, true);
resetDetectionClassifications();
runObjectClassifier("p53");
